...
Этот раздел описывает инфраструктуру приложения, развернутого в Kubernetes-кластере в Yandex Cloud.  
...
Документация основана на реальной топологии из Yandex Cloud и структуре репозитория, включающего Terraform-модули и плейбуки Ansible.  
...
Terraform управляет жизненным циклом:
...
| **Kubernetes**          | 1.30.x            | Ansible → роль `kubeadm` / конфиг kubeadm |
| **kubeadm**             | 1.30.x            | Ansible → роль `kubeadm`                  |
| **kubelet**             | 1.30.x            | Ansible → роль `kubeadm`                  |
| **kubectl**             | 1.30.x            | Ansible → роль `kubeadm`                  |
| **CNI (Calico)**        | v3.x              | Установка manifest-ом в `init_master.yml` |
...
| **cloud-init**          | Стандарт YC       | Виртуальная машина Yandex Cloud           |


## Результат

После выполнения обоих этапов вы получаете:

* полностью работоспособный Kubernetes-кластер;
* автоматизированную развёртку инфраструктуры;
* кластер, распределённый по двум зонам доступности;
* чистую, декларативную и воспроизводимую IaC-архитектуру.
