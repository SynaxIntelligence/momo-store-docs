...
Для развертывания приложения используется набор Helm-чартов в каталоге `charts/`.  
Он включает корневой (umbrella) чарт и два дочерних чарта — для backend и frontend.
...
- **prod**
    ...
    - для prod обычно повышены лимиты ресурсов, включён HPA/VPA и настроены боевые домены в ingress.
...
3. Внешний CD-контур (например, Argo CD) применяет изменения к кластеру.
